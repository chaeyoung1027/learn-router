...
const RedirectComponent = (props) => {
    ...
    if(props.redirectPath) {
        ...
        */
        ...
        /* 
        - 렌더링하는 과정에서 리다이렉트를 하고 싶은 경우는 Navigate컴포넌트를 그려줌 
        => 사실 렌더링 x, Navigate 컴포넌트의 to 속성으로 전달한 주소로 리다이렉트 작업 수행
        - 렌더링이 모두 끝난 후 이벤트 핸들러 (ex. click Event)에 의해서 리다이렉트를 하고 싶은
        경우는 useNavigate 훅을 통해 전달받은 navigate함수를 호출하여 리다이렉트 작업을 수행한다.
        */
    } else {
        return <div>
            <button onClick={() => {
